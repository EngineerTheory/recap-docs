...
## RecapBot Introduction

Recap will connect to your Slack and listen for messages to answer.

You can easily configure rules for what channels Recap should respond in, what knowledge sets should back each
configured channel, and set filters to respond or not respond to different types of messages.

When Recap identifies valid questions, it will respond in the message thread with:
...
Recap also provides a configuration page so you can create custom settings for each slack channel
(or you can just use the default settings for all your channels).

![SlackBotConfig](/images/slack_bot/SlackBotConfig.png)

Since we are using Web Sockets, Recap is able to initiate the connection. This means that this is able
to work even if you are running Recap inside a firewall protected VPC.
...
![RecapBotInstall](/images/slack_bot/RecapBotInstall.png)
9. Copy the **Bot User OAuth Token**, this will be needed later to enable us to respond to messages as RecapBot.

### Setting Recap to use it

Start by finding the admin page in the top right of the Recap web UI.
![RecapAdminButton](/images/slack_bot/RecapAdminButton.png)

On the left hand menu of the admin page, navigate to the bottom to **Bots**.

Provide your Slack Tokens and Recap will start to respond to your slack channels after a short startup delay.
...
[here](https://github.com/Recap-ai/Recap/blob/main/backend/Recap/configs/Recapbot_configs.py)
