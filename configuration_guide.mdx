...
There are defaults set in the code so changing/settings these values is **not required** to use Recap.
...
- WEB_DOMAIN (your full url in production, including the protocol- e.g. `https://www.Recap.ai`)
...
**the preferred approach** for Recap is to use the `.env` file. To do this, create a file called `.env` at
`Recap/deployment/docker_compose/.env`. From there, populate it with the values you want to override:
...
[here](https://github.com/Recap-ai/Recap/blob/main/deployment/kubernetes/env-configmap.yaml).

## All Environment Variables


This is an extensive list of the currently supported environment variables within Recap. There are several classes of environment variables. All of the global configuration options that are not built into the UI are set via environment variables.
...
These variables control authentication and user management in Recap.

#### AUTH_TYPE
Controls the authentication method used by Recap.
...
When set to `true`, users must verify their email before accessing Recap.
...
These variables configure the generative AI capabilities of Recap. These are covered in more depth at the [generative AI configs](/gen_ai_configs/overview).
...
Configuration for external services used by Recap.
...
#### WEB_DOMAIN
Fully qualified domain name used for the Recap web interface. (e.g. https://www.Recap.com)
...
#### DISABLE_TELEMETRY
Set to `true` to opt out of telemetry. Telemetry helps improve Recap; no sensitive data is collected.
