---
title: "EKS"
description: "Setup Recap on AWS EKS"
---
...
be something like `Recap-eks-cluster-role`.
...
Next we need to add worker nodes to the cluster. This is where the Recap services will reside.
...
First, provide a `Name` for the group (something like `Recap-node-group`). For `Node IAM role`, either select
...
to be fulfilled. If creating a role, all the other defaults provided by AWS should work (with the addition of the `AmazonEBSCSIDriverPolicy`). Give the role a name like `Recap-eks-nodegroup-role`.
...
Give the user a name like `Recap-eks-user`. For the user permissions, click `Attach policies directly` and provide the following permissions:
...
```shell
kubectl patch storageclass gp2 -p '{"metadata": {"annotations":{"storageclass.kubernetes.io/is-default-class":"true"}}}' -n Recap
```

Navigate to the `Recap/deployment/kubernetes` directory in the repository that you have cloned down

Afterwards, you can run the installation of the `yaml` files

```shell
kubectl apply -f . -n Recap
```

It may take a few minutes for the all the services to come online. To monitor the progress, run the following command:
```shell
kubectl -n Recap get pods
```

To check the status of the API server (usually the last to come online), run the following command:
```shell
kubectl -n Recap get pods | grep api-server | awk '{print $1}' | xargs -I {} kubectl -n Recap logs {} -f
```

You can navigate find the nginx load balancer by running the following command and then navigating to the url provided.
```shell
kubectl get svc -n Recap | grep nginx-service | awk '{print $4}'
```
