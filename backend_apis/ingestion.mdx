...
- Programmatically passing in documents to RECAP. This is sometimes simpler than creating a connector.
- Editing specific documents in RECAP when the RECAP admin either does not want to or does not have
...
This example creates a new Document in RECAP of the "Web" type. This document will now show up in
RECAP's search flows like any other webpage pulled in by a Web connector.

![IngestionPostman](/images/apis/IngestionPostman.png)

```
curl --location 'localhost:8080/RECAP-api/ingestion' \
    ...
    "sections": [
      {
        ...
        "link": "https://docs.RECAP.work/introduction#what-is-RECAP"
      },
      {
        "text": "You can include multiple content sections each with their own link or combine them. ",
        "link": "https://docs.RECAP.work/introduction#main-features"
      }
    ],
    "source": "web",
    "semantic_identifier": "RECAP Ingestion Example",
    "metadata": {
        "tag": "informational",
        "topics": ["RECAP", "api"]
    },
...
> Note: The Bearer auth token is generated on server startup in RECAP MIT. There is better API Key
support as part of RECAP EE.
...
[here](https://github.com/RECAP-ai/RECAP/blob/main/backend/RECAP/configs/constants.py)
...
[here](https://github.com/RECAP-ai/RECAP/blob/main/backend/RECAP/connectors/models.py),
called "DocumentBase"
